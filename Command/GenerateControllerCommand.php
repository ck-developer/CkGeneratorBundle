...
use Ck\Bundle\GeneratorBundle\Generator\ControllerGenerator;
    ...
    public function configure()
    {
        $this
            ->setDefinition(array(
                ...
                new InputOption('office', '', InputOption::VALUE_OPTIONAL, 'The name of the controller to create'),
...
EOT
            ...
            ->setName('ck:generate:controller')
        ;
    }
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $generator->generate($bundle, $controller, $input->getOption('route-format'), $input->getOption('template-format'), $this->parseActions($input->getOption('actions')), $input->getOption('office'));
    ...
    public function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        // office controller
        $defaultOffice = (null !== $input->getOption('office') ? $input->getOption('office') : 'default');
        $output->writeln(array(
            '',
            'Determine the office controller.',
            '',
        ));
        $question = new Question($questionHelper->getQuestion('Office controller (backend, frontend, api)', $defaultOffice), $defaultOffice);
    ...
    public function addActions(InputInterface $input, OutputInterface $output, QuestionHelper $questionHelper)
    {
        ...
        while (true) {
            ...
            // template
            if ('default' == $input->getOption('office')) {
                $defaultTemplate = $input->getOption('controller').':'.substr($actionName, 0, -6).'.html.'.$input->getOption('template-format');
            } else {
                $office = ucfirst($input->getOption('office'));
                $defaultTemplate = $input->getOption('controller').':'.$office.'/'.substr($actionName, 0, -6).'.html.'.$input->getOption('template-format');
            }
