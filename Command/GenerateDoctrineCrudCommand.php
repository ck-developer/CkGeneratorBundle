...
use Ck\Bundle\GeneratorBundle\Generator\DoctrineCrudGenerator;
    ...
    protected function configure()
    {
        $this
            ->setDefinition(array(
                ...
                new InputOption('office', '', InputOption::VALUE_OPTIONAL, 'Do not stop the generation if crud controller already exist, thus overwriting all generated files'),
            ))
...
<info>BUNDLE_PATH/Resources/CkGeneratorBundle/skeleton/crud
APP_PATH/Resources/CkGeneratorBundle/skeleton/crud</info>

And

<info>__bundle_path__/Resources/CkGeneratorBundle/skeleton/form
__project_root__/app/Resources/CkGeneratorBundle/skeleton/form</info>

You can check https://github.com/sensio/CkGeneratorBundle/tree/master/Resources/skeleton
in order to know the file structure of the skeleton
EOT
            )
            ->setName('ck:generate:crud')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $generator->generate($bundle, $entity, $metadata[0], $format, $prefix, $withWrite, $forceOverwrite, $office);
        ...
        if ($withWrite) {
            ...
            if ($this->generateForm($bundle, $entity, $metadata, array('edit', 'create'))) {
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $question->setValidator(array('Ck\Bundle\GeneratorBundle\Command\Validators', 'validateEntityName'));
        ...
        // office
        $defaultOffice = (null !== $input->getOption('office') ? $input->getOption('office') : 'backend');
        $output->writeln(array(
            '',
            'Determine the office controller.',
            '',
        ));
        $question = new Question($questionHelper->getQuestion('Office controller (backend, frontend, default)', $defaultOffice), $defaultOffice);
        $question->setValidator(array('Ck\Bundle\GeneratorBundle\Command\Validators', 'validateOffice'));
        $officeController = $questionHelper->ask($input, $output, $question);
        $input->setOption('office', $officeController);
        ...
        $question->setValidator(array('Ck\Bundle\GeneratorBundle\Command\Validators', 'validateFormat'));
     ...
     */
    protected function generateForm($bundle, $entity, $metadata, $office)
    {
        try {
            $this->getFormGenerator($bundle)->generate($bundle, $entity, $metadata[0], array('edit', 'create'));
