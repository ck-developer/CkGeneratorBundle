...
class ControllerGenerator extends Generator
{
    ...
    public function generate(BundleInterface $bundle, $controller, $routeFormat, $templateFormat, array $actions = array(), $office)
    {
        $dir = $bundle->getPath();
        ...
        if('default' == $office)
        {
            ...
            $controllerTestFile = $dir.'/Tests/Controller/'.$controller.'ControllerTest.php';
        }
        else {
            $office = ucfirst($office);

            $controllerFile = $dir.'/Controller/'.$office.'/'.$controller.'Controller.php';
            $controllerTestFile = $dir.'/Tests/Controller/'.$office.'/'.$controller.'ControllerTest.php';
        }
        ...
        foreach ($actions as $i => $action) {
            ...
            if ('default' == $template && null == $office) {
                $template = $bundle->getName().':'.$controller.':'.substr($action['name'], 0, -6).'.html.'.$templateFormat;
            }
            else {
                $template = $bundle->getName().':'.$office.'/'.$controller.':'.substr($action['name'], 0, -6).'.html.'.$templateFormat;
            }

            $actions[$i]['template'] = $template;
        ...
        $this->renderFile('controller/ControllerTest.php.twig', $controllerTestFile, $parameters);
    }
